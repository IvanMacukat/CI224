...
int main(int argc, char ** argv) {
	...
	window = SDL_CreateWindow("CI224 - Game", SDL_WINDOWPOS_CENTERED,
	...
	// Set the camera
	Camera::getInstance().lookAt(Point3(0.0, 0.0, -8.0),
			Point3(0.0, 0.0, -15.0), Vector3(0.5, 1.0, 0.5));
	...
	while (SDL_WaitEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			case SDLK_LEFT:
				Camera::getInstance().setCamera(
						(camera * Matrix4::rotationY(5.0 / 180.0)));
				break;
			case SDLK_RIGHT:
				Camera::getInstance().setCamera(
						camera * Matrix4::rotationY(-5.0 / 180.0));
			...
			case SDLK_UP:
				Camera::getInstance().setCamera(
						camera * Matrix4::translation(Vector3(0.0, 0.0, -1.0)));
				break;
			case SDLK_DOWN:
				Camera::getInstance().setCamera(
						camera * Matrix4::translation(Vector3(0.0, 0.0, 1.0)));
