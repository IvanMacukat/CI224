...
void GameAsset::common_init() {
	bbox = shared_ptr<BoundingBox>(
			new BoundingBox(Point3(0, 0, 0), 1.0, 1.0, 1.0)); // unit cube
}
...
void GameAsset::draw() {
	...
	glVertexAttribPointer(position_attrib, /* attribute */
	3, /* size */
	GL_FLOAT, /* type */
	GL_FALSE, /* normalized? */
	0, /* stride */
	...
	glVertexAttribPointer(faceColor_attrib, 3, GL_FLOAT, GL_FALSE, 0, (void*) 0);

	glEnableVertexAttribArray(position_attrib);
	glEnableVertexAttribArray(faceColor_attrib);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, element_buffer);
	glDrawElements(
	GL_TRIANGLES, 3 * this->num_triangles,
	GL_UNSIGNED_SHORT, (GLvoid*) 0);

	glDisableVertexAttribArray(position_attrib);
	glDisableVertexAttribArray(faceColor_attrib);
}

/*
 * Functions for general purpose stuff
 */
GLchar * GameAsset::shader_file_contents(const string &filename,
		GLint * length) {
	ifstream input_file;
 ...
 */
GLuint GameAsset::make_buffer(GLenum target, const void *buffer_data,
		GLsizei buffer_size) {
	GLuint buffer;
...
GLuint GameAsset::make_shader(GLenum type, const char *filename) {
	...
	if (!shader_ok) {
		cerr << "Failed to compile " << filename << " with error code "
				<< shader_ok << endl;
...
int GameAsset::make_resources(void) {
	...
	faceColor_attrib = glGetAttribLocation(program, "faceColor");
